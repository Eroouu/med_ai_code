...
class MedicalInterviewBot:
    ...
    async def generate_symptoms_question(self, session_id: str) -> str:
        ...
        try:
            response = self.llm.invoke(prompt)  # ← ГЛАВНОЕ ИЗМЕНЕНИЕ
    ...
    async def should_stop_conversation(self, session_id: str) -> bool:
        ...
        Ответь ТОЛЬКО "ДА" или "НЕТ" без точек или запятых в конце """

        try:
            response = await asyncio.to_thread(self.llm.invoke, prompt) 
            print(response.content.strip().upper())
    ...
    async def generate_text_report(self, session_id: str) -> str:
        """Генерирует связный медицинский текстовый отчет с помощью нейросети"""
        try:
            session = self.load_session(session_id)
            if not session:
                return ""  # Возвращаем пустую строку, а не ошибку
            
            demo = session.get("demographics", {})
            gen_q = session.get("general_questions", {})
            symp = session.get("symptoms", {})
            dialog =""
            conversation = symp.get("conversation", [])
            for msg in conversation:
                role = "Врач" if msg["role"] == "bot" else "Пациент"
...
Пациент:
- Имя: {demo.get('name', 'не указано')}
- Возраст: {demo.get('age', 'не указано')} лет
- Вес: {demo.get('weight', 'не указано')} кг
- Рост: {demo.get('height', 'не указано')} см

Общие вопросы:
- Какие пациент принимает лекарства: {gen_q.get('medications', 'не указано')}
- Аллергии: {gen_q.get('allergies', 'не указано')}
- Операции: {gen_q.get('surgeries', 'не указано')}
- Хронические заболевания: {gen_q.get('chronic_diseases', 'не указано')}
- Образ жизни: {gen_q.get('lifestyle', 'не указано')}

Жалобы:
{symp.get('chief_complaint', 'не указано')}:

"""

            # Формируем текст с данными для промта
            prompt = dialog


            prompt += """
            Ты профессиональный врач, который составляет медицинские отчеты.
            На основе диалога с пациентом составь структурированный и подробный медицинский отчет с разделами:

            1) Основная жалоба
            (то есть перефразируй основную жалобу пациента в медицинский стиль)

            2) История настоящего заболевания
            (Расскажи подробно о симптомах, их развитии, интенсивности, провоцирующих факторах)

            ВАЖНО: не надо писать ничего другого, только эти два раздела. Без названия файла или прочей информации.То есть 
            не надо писать время приема или ID сессии и тд. Только эти два раздела.

            Символ _ внутри формата, означает место для заполнения данными пациента.
            ...
            Основная жалоба пациента:
            У пациента сильно болит голова и горло

            История настоящего заболевания:

            В последние 3 дня у пациента наблюдались следующие симптомы:
            - Боль в горле;
            - Головная боль высокой интенсивности, описываемая как "раскалывание";
            - У пациента возникла высокая температура, отмеченная начиная с момента начала болезни.

            Прочие симптомы отсутствуют. Пациент не испытывал головокружений или шума в ушах при болях, а также не чувствовал чувствительности к свету или ярким цветам.

            """

            # Запрос к нейросети
            response = await asyncio.to_thread(self.llm.invoke, prompt)
            report_text = base_report + response.content.strip() + dialog
            
            if not report_text or len(report_text) < 50:
                # Если отчет слишком короткий, вернуть пустую строку
                return ""
            
            return report_text
        
        except Exception as e:
            print(f"❌ Ошибка генерации отчета: {type(e).__name__}: {e}")
            import traceback
            traceback.print_exc()
